...
```{python}
ds.gen_graph(year=2022, var="insecurity_hous",type='linear', title="asdfsdf")
```
...
df2 = df.filter(pl.col("year")==2021)
df2
```
...
```{python}
alt.Chart(master).mark_geoshape().encode(
    color=alt.Color("total_insec:Q").scale(scheme='magma')
).transform_lookup(
    lookup='geoid',
    from_=alt.LookupData(master, 'geoid', ['total_insec'])
).project(
    type='mercator'
).properties(
    width="container",
